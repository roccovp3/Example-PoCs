...
#define LINE_SIZE 256      // 128B cache line, as you described
...
int main(int argc, char *argv[])
{
    ...
    size_t smart_evict_bytes = BUF_SIZE_MB * 1024 * 1024;
    ...
    printf("Evict ptrs shuffled\n");

    uint32_t rep = 100;  // number of trials per N


    const size_t page_size  = 4096;      // or (size_t)sysconf(_SC_PAGESIZE);
    const size_t cache_line = 256;        // actual cache line size
    ...
    *data = 42;
    ...
    for (int j = 0; j <= 40000; j += 100) {
        printf("Testing with j = %d\n", j);
        int cache_miss = 0;
        int cache_hit  = 0;
        int time_sum = 0;
        ...
        for (uint32_t n = 0; n < rep; n++) {
            ...
            // printf("Trial %u: access time = %d cycles (%s)\n", n, time,
            //        (time > threshold) ? "miss" : "hit");
        }
        double avg_time = (double)time_sum / (double)rep;
        printf("Average access time over %u trials: %.2f cycles\n", rep, avg_time);

        // Re-shuffle for the next j so we change which lines are in the first j
        printf("Cache misses: %d out of %d trials\n", cache_miss, rep);

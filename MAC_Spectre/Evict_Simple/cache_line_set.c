...
size_t pop_cache_line_from_set_by_index(cache_line_set_t *cache_line_set, int index) {
    if ( __builtin_expect(index < 0 || index >= cache_line_set->num_cache_lines, 0) ) {
        fprintf(stderr, "ERROR: cannot pop index %d from cache line set of size %ld\n", index, cache_line_set->num_cache_lines);
        exit(1);
    }
...
cache_line_set_t* copy_cache_line_set(cache_line_set_t* cache_line_set_to_copy) {
    ...
    for (int i = 0; i < new_cache_line_set->num_cache_lines; i++)
        new_cache_line_set->cache_lines[i] = cache_line_set_to_copy->cache_lines[i];
...
cache_line_set_t* reduce_cache_line_set(cache_line_set_t* orig_cache_line_set, int num_elem) {
    ...
    if ( __builtin_expect(num_elem > orig_cache_line_set->num_cache_lines, 0) ) {
        fprintf(stderr, "ERROR: cannot reduce a cache line set of size %ld into %d size!\n", orig_cache_line_set->num_cache_lines, num_elem);
        exit(1);
    }
...
cache_line_set_t* merge_two_cache_line_sets(cache_line_set_t* set1, cache_line_set_t* set2) {
    cache_line_set_t* merged_cache_line_set = build_empty_cache_line_set(NULL);

    for (int i = 0; i < set1->num_cache_lines; i++)
        push_cache_line_to_set(merged_cache_line_set, set1->cache_lines[i]);

    for (int i = 0; i < set2->num_cache_lines; i++)
        push_cache_line_to_set(merged_cache_line_set, set2->cache_lines[i]);
...
void print_cache_line_set(cache_line_set_t* cache_line_set) {
    printf("cache line set size: %ld\n", cache_line_set->num_cache_lines);
    for (int i = 0; i < cache_line_set->num_cache_lines; i++)
        printf("cache line #%d: 0x%lx  |xxx|0x%lx|0x%lx|0x%lx|\n", 
